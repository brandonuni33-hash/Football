    ...
    simulateBlock(state, trainingFocus = 'TECHNIQUE', userMatchChoice = null) {
        ...
        const results = Array.from({ length: matchesInMonth }, (_, matchIndex) => {
            ...
            return {
                ...
                round: scheduledMatch?.round || null,
        ...
        // La Coupe nationale utilise la performance du joueur pour le score
        // de son club. Les autres matchs de la journée sont ensuite simulés
        // normalement, sans polluer le classement du championnat.
        let cupResult = null;
        const cupMatchIndex = scheduledMatches.findIndex(
            match => match.competitionType === 'national_cup'
        );
        const cupMatch = cupMatchIndex >= 0 ? scheduledMatches[cupMatchIndex] : null;

        if (cupMatch) {
            cupResult = CupSystem.resolvePlayerMatch(
                state,
                cupMatch,
                results[cupMatchIndex] || {}
            );
            ...
            if (
                cup &&
                cup.status === 'active' &&
                Number(cup.roundMonth) === Number(calendar.currentMonth) &&
                cup.matches?.length
            ) {
                CupSystem.simulateCurrentRound(state);
            }
