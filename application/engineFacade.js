...
export function bindEngineToRegistry(engine, registry) {
    ...
    // Match interactif : le calendrier n'avance pas tant que le match n'est pas terminé.
    engine.startInteractiveMatch = (scheduledMatch, matchIndex = 0) => {
        const manager = registry.matchBlockManager;
        if (!manager?.startInteractiveMatch) throw new Error('Interactive MatchBlock indisponible.');
    ...
    engine.getSuccessorOptions = () => {
        if (!engine.state?.player?.id) return [];
        return registry.generationSimulationFacade.getOptions({ state: engine.state, playerId: engine.state.player.id, currentAge: engine.state.player.age });
    };
    engine.simulateSuccessorTo14 = (childId) => {
        if (!engine.state?.player?.id) return null;
        return registry.generationSimulationFacade.simulateTo14({ state: engine.state, playerId: engine.state.player.id, childId, currentAge: engine.state.player.age, world: engine.state.world || {} });
    };
    engine.startSuccessorCareer = (childId) => {
        if (!engine.state?.player?.id) return null;
        return registry.generationSimulationFacade.startIfReady({ state: engine.state, playerId: engine.state.player.id, childId, currentAge: engine.state.player.age, world: engine.state.world || {} });
    };
    engine.resetCareer = () => {
        const result = registry.careerLifecycleSystem.reset();
        engine.state = null;
        if (engine.ui) {
            engine.ui.activeApp = 'home'; engine.ui.currentStep = 1;
            engine.ui.selectedData = { firstname:'', lastname:'', position:null, continent:null, country:null, origin:null, heartClub:null, youthClub:null, coachVision:null, coachName:null };
            engine.ui.render?.();
        }
        return result;
    };
    ...
    engine.__architecture = Object.freeze({
        phase: 6,
        delegated: ['startCareer','playBlock','advanceCalendar','getPeriodName','setTrainingFocus','resolveEventChoice','resolveCoachChoice','resolveMediaDilemma','resolvePositionProposal','acceptTransferOffer','rejectTransferOffer','retireCareer','startInteractiveMatch','resolveInteractiveMatchDecision','getScheduledMatches','resetCareer','registerChildBirth','getChildren','getSuccessorOptions','simulateSuccessorTo14','startSuccessorCareer']
    });
    return engine;
}
