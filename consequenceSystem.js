...
// Principe : un choix produit toujours une réaction narrative immédiate,
// puis peut laisser une trace différée dans la carrière. Les valeurs chiffrées
// restent cachées au joueur.
...
function buildImmediateReaction(choice, normalized, source) {
    ...
    if (has('relationCoach')) {
        return negative('relationCoach')
            ? 'Le coach ne répond pas tout de suite. Son regard laisse pourtant comprendre qu’il n’a pas vraiment apprécié ta décision.'
            : 'Le coach acquiesce. Il semble avoir apprécié ta façon de gérer la situation.';
    }
    if (has('vestiaire')) {
        return negative('vestiaire')
            ? 'Quelques regards changent autour de toi. La décision ne fait visiblement pas l’unanimité dans le vestiaire.'
            : 'L’ambiance autour de toi semble s’améliorer. Certains coéquipiers apprécient clairement ton choix.';
    }
    if (has('fitness') || has('injuryDuration') || has('isInjured')) {
        return negative('fitness') || has('isInjured')
            ? 'Tu assumes ta décision, mais ton corps risque de te rappeler que chaque choix a un prix.'
            : 'Tu prends une décision prudente. Le staff semble rassuré par ton attitude.';
    }
    if (has('balance')) {
        return 'La décision a un coût concret. Tu assumes la dépense sans savoir encore ce qu’elle changera pour la suite.';
    }
    if (has('fame') || has('reputation') || has('media.hypeLevel') || has('media.followers')) {
        return positive('fame') || positive('reputation') || positive('media.hypeLevel')
            ? 'Ta décision commence à circuler. Pour l’instant, les réactions sont plutôt favorables.'
            : 'Ta décision attire quelques réactions. Il faudra voir comment la situation évolue.';
    }
    if (has('discipline')) {
        return negative('discipline')
            ? 'Tu sais que le staff a remarqué ton attitude. La suite dépendra surtout de ce que tu feras maintenant.'
            : 'Ton attitude est remarquée. Le staff semble apprécier ton sérieux.';
    }
...
function addMemory(state, entry) {
    ...
    state.careerMemory.push({
        ...
        immediateReaction: entry.immediateReaction,
        trace: entry.trace,
        status: 'active'
    });
...
function queueEffect(state, choiceId, effect, options = {}) {
    ...
    return {
        id: createId(), choiceId, source, target,
        ...
        visibility: visibilityFor(target, magnitude),
        label: effect.label || LABELS[target] || target || 'Conséquence',
        trigger: effect.trigger || 'block_start',
        createdAt: new Date().toISOString(), resolved: false, revealed: false
    };
}
...
function queueChoice(state, choice, options = {}) {
    ...
    const trace = queued.length
        ? queued.map(item => ({ id: item.id, visibility: item.visibility, hint: memoryLabel(item.target, item.magnitude) }))
        : [{ id: createId('trace'), visibility: 'indirect', hint: 'Cette décision pourra influencer la suite de ta carrière.' }];

    const memoryEntry = {
        id: createId('memory'), choiceId, source: options.source || 'Choix',
        choiceText: choice.text || choice.texte || choice.label || normalized.title || 'Décision',
        immediateReaction, trace, createdAt: new Date().toISOString()
    };
    addMemory(state, memoryEntry);
    state.lastChoiceReaction = {
        choiceId,
        source: memoryEntry.source,
        text: memoryEntry.choiceText,
        reaction: immediateReaction,
    ...
    return {
        ...
        responseText: immediateReaction,
        immediateReaction,
        ...
        changes: [],
        temporary: [],
        memory: { recorded: true, traceCount: trace.length }
    };
}
...
function applyStateTarget(state, consequence) {
    ...
    if (target?.startsWith?.('matchBonus.')) {
        player.temporaryEffects.push({ id: consequence.id, stat: target, value, duration: consequence.duration, remainingMatches: consequence.duration, source: consequence.source, label: consequence.label });
...
function resolvePending(state, trigger = 'block_start') {
    ...
    for (const consequence of state.consequences || []) {
        ...
        const hint = change?.delta > 0
            ? `Tu commences à voir les effets positifs d’une décision prise plus tôt.`
            : change?.delta < 0
                ? `Tu commences à voir les effets d’une décision prise plus tôt.`
                : `Une décision passée semble avoir laissé une trace.`;
        if (memory) {
            memory.status = 'resolved';
            memory.resolution = { visibility: consequence.visibility, hint, resolvedAt: consequence.resolvedAt };
        }
        ...
        revealed.push({
            ...
            narrative: hint,
            result: consequence.visibility === 'hidden' ? null : change
        });
    }
