...
function formatChanges(result) {
    ...
    if (number(result?.xp) > 0) {
        lines.push(`<div class="stp-list-item"><strong>Expérience</strong><span>+${number(result.xp)} XP</span></div>`);
    }
    ...
    app.insertAdjacentHTML('beforeend', `
        <div class="stp-modal" id="stp-result-modal">
            <div class="stp-modal-card">
                ...
                ${changesHTML
                    ? `<h3 class="stp-section-title" style="margin-top:8px">Ce qui change</h3><div class="stp-list">${changesHTML}</div>`
                    : `<p class="stp-modal-text">Aucune conséquence chiffrée immédiate n'est affichée.</p>`}
...
UserInterface.prototype.playBlockSafely = function(choice = null) {
    // Premier clic : s'il existe un match et qu'un dilemme doit être déclenché,
    // on demande la décision avant de simuler le bloc.
    ...
    if (choice) {
        // Le moteur historique attend directement l'objet de choix.
