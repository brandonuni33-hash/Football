...
const CompetitionSystem = {
    ...
    getYouthCategory(age) { if (age <= 15) return 'U15'; if (age === 16) return 'U16'; if (age === 17) return 'U17/U19'; if(age<=20)return'U20'; return null; },
    ...
    createSeasonSchedule(player, seasonYear) { const generated = buildSeasonSchedule(player, seasonYear, this.getSeniorCompetition.bind(this), buildEuropeanLeague, getEuropeanQualification); return this.finalizeSchedule(player, seasonYear, generated.matches, generated.category, generated.seed); },
    finalizeSchedule(player, seasonYear, matches, category, seed) { const ordered = sortMatches(matches), byMonth = {}; for (const month of ALL_MONTHS) byMonth[month] = { month, label: this.getMonthLabel(month), period: this.getPeriodName(month), phase: MONTH_INFO[month]?.phase || 'offseason', matches: [] }; ordered.forEach(match => byMonth[match.month].matches.push(match)); return { version: 5, seasonYear, seasonLabel: seasonLabel(seasonYear), generatedForAge: Number(player?.age) || 14, category, seed, matches: ordered, byMonth, totals: { allMatches: ordered.length, leagueMatches: ordered.filter(match => match.type === 'league' || match.type === 'youth').length, cupMatches: ordered.filter(match => match.competitionId === 'NATIONAL_CUP').length, europeanMatches: ordered.filter(match => ['CHAMPIONS_LEAGUE', 'EUROPA_LEAGUE'].includes(match.competitionId)).length, youthLeagueMatches:ordered.filter(match=>match.competitionId==='YOUTH_LEAGUE').length, u20WorldCupMatches:ordered.filter(match=>match.competitionId==='U20_WORLD_CUP').length } }; },
    ensureSeasonSchedule(state) { if (!state?.player || !state?.calendar) return null; const year = Number(state.calendar.currentSeasonYear) || new Date().getFullYear(), existing = state.calendar.seasonSchedule; if (existing && Number(existing.seasonYear) === year && Array.isArray(existing.matches) && Number(existing.version || 0) >= 5) return existing; CupSystem.ensure(state); const schedule = this.createSeasonSchedule(state.player, year); state.calendar.seasonSchedule = schedule; state.calendar.seasonMatchCursor = 0; const competition = this.getSeniorCompetition(state.player), qualification = getEuropeanQualification(state.player, competition); state.europeanTournament = Number(state.player.age) >= 18 ? createEuropeanTournament(state.player, year, qualification, schedule.seed) : null; return schedule; },
    ...
    getBlockPlan(state) {
        ...
        // Juin/juillet restent une intersaison uniquement s'il n'y a aucun tournoi international programmé.
        // Cela permet notamment à la Coupe du Monde U20 de se jouer sans rendre les mois d'été compétitifs par défaut.
        const scheduledSummerMatches=(monthData?.matches||[]).filter(match=>!match.played);
        if (this.isOffSeason(month) && !scheduledSummerMatches.length) return { type: 'offseason', month, monthLabel: this.getMonthLabel(month), season: seasonYear, seasonLabel: seasonLabel(seasonYear), matches: 0, scheduledMatches: [], activities: month === 7 ? ['repos', 'mercato', 'programme_individuel', 'preparation_saison'] : ['bilan', 'selection_internationale', 'repos', 'recovery'], importance: 'none', mode: 'career_activity' };
        const base = [...(monthData?.matches || []).filter(match=>!match.played), ...CupSystem.getPlayerFixtures(state)], europeanFixture = this.getEuropeanFixture(state); if (europeanFixture && !base.some(match => match.id === europeanFixture.id)) base.push(europeanFixture);
