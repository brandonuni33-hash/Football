...
import { appendGoalEvent,deriveGoalSummary,GOAL_ACTOR_TYPE,GOAL_TEAM_SIDE,PLAYER_GOAL_CONTRIBUTION } from './goalEventResolver.js';
...
function extraTimeActor(session,side){return side===GOAL_TEAM_SIDE.PLAYER?{actorType:GOAL_ACTOR_TYPE.TEAMMATE,actorId:`${session.id}:teammate`,displayName:'Un coéquipier'}:{actorType:GOAL_ACTOR_TYPE.OPPONENT,actorId:`${session.id}:opponent`,displayName:`Un joueur de ${session.opponent}`};}
function applyExtraTimeGoals(session,resolution){let teamMissing=Math.max(0,n(resolution.teamGoals)-ownScore(session)),opponentMissing=Math.max(0,n(resolution.opponentGoals)-oppScore(session)),minute=Math.max(91,n(session.goalEvents?.at(-1)?.minute)+1);while(teamMissing>0||opponentMissing>0){const side=teamMissing>0?GOAL_TEAM_SIDE.PLAYER:GOAL_TEAM_SIDE.OPPONENT;appendGoalEvent(session,{minute:Math.min(120,minute),teamSide:side,scorer:extraTimeActor(session,side),assist:null,source:{type:'EXTRA_TIME'},playerContribution:PLAYER_GOAL_CONTRIBUTION.NONE});if(side===GOAL_TEAM_SIDE.PLAYER)teamMissing-=1;else opponentMissing-=1;minute+=7;}return deriveGoalSummary(session.goalEvents,{home:session.home,playerId:session.playerId});}
function applyResolution(session,resolution) { session.knockoutResolution=resolution;const summary=deriveGoalSummary(session.goalEvents||[],{home:session.home,playerId:session.playerId});session.score={...summary.score};if(session.result){session.result.regulationScore=resolution.regulationScore;session.result.wentToExtraTime=resolution.wentToExtraTime;session.result.wentToPenalties=resolution.wentToPenalties;session.result.penaltyScore=resolution.penaltyScore;session.result.shootoutWinner=resolution.shootoutWinner||null;session.result.goalEvents=(session.goalEvents||[]).map(event=>({...event,scorer:{...event.scorer},assist:event.assist?{...event.assist}:null,source:{...event.source}}));Object.assign(session.result,summary);const won=resolution.wentToPenalties?resolution.shootoutWinner==='team':summary.teamGoals>summary.opponentGoals;session.result.result=won?'win':'loss';if(playerStillOnPitch(session))session.result.minutesPlayed=Math.max(n(session.result.minutesPlayed),120);} }
...
export function advanceInteractiveMatch(state,activeSession,action={}){
    ...
    if(session?.knockoutRuntimeStage==='extra_time_intro'){const resolution=session.knockoutResolution;applyExtraTimeGoals(session,resolution);session.knockoutRuntimeStage='extra_time_end';session.step=extraTimeEnd(session);return enrichProfessionalExperience(state,session,{finished:false,session,step:session.step,decision:null,result:session.result});}
