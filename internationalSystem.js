// internationalSystem.js
// Equipes nationales : Euro + Coupe du Monde.
// Le système est volontairement indépendant de l'UI afin de limiter les risques
// de régression dans le dashboard.
...
const COUNTRIES = [
    ...
    'Portugal', 'Pays-Bas', 'Belgique', 'Croatie', 'Brésil',
    'Argentine', 'Uruguay', 'Colombie', 'Mexique', 'États-Unis',
    'Japon', 'Corée du Sud', 'Maroc', 'Sénégal', 'Suisse'
];

const clamp = (v, min, max) => Math.min(max, Math.max(min, Number(v) || 0));

function tournamentForYear(year) {
    // Cycle volontairement simple et stable pour le mode carrière.
    // Euro : années paires divisibles par 4 ; Mondial : années paires non divisibles par 4.
...
function simulateResult(player, fixture) {
    ...
    const ownGoals = Math.min(5, Math.max(0, Math.round(1.25 + diff / 30 + (random() - 0.5) * 2)));
    const opponentGoals = Math.min(5, Math.max(0, Math.round(1.1 - diff / 35 + (random() - 0.5) * 2)));
    return { ownGoals, opponentGoals };
}

function isPlayerEligible(player) {
    return !!player && Number(player.age) >= 17 && !!(player.country || player.nationality);
}
...
export const InternationalSystem = {
    TOURNAMENTS,
        ...
        const fixtures = opponents.map((opponent, index) => makeFixture(
            tournament, year, country, opponent, 'group', 'Phase de groupes', tournament.groupMonths[0], index, random
        ));
        ...
        state.international.current = {
            ...
            phase: 'group',
            group: { played: 0, points: 0, goalsFor: 0, goalsAgainst: 0 },
            fixtures,
            history: [],
            qualified: false,
            eliminated: false,
    ...
    getCurrent(state) {
        this.ensure(state);
        return state?.international?.current || null;
    },
    ...
    getPlayerFixture(state) {
        ...
        return current.fixtures.find(f => !f.played && Number(f.month) === month) || null;
    },
        ...
        if (current.group.played >= 3) {
            ...
            if (!current.qualified) {
                ...
                current.fixtures.push(makeFixture(
                    ...
                    current.seasonYear,
                    current.nationalTeam,
                    opponentPool(current.nationalTeam)[0],
                    'knockout',
                    knockout[0],
                    6,
                    current.fixtures.length,
                    seededRandom(hashSeed(`${current.seasonYear}|${current.nationalTeam}|ko`))
                ));
            }
            ...
            if (index >= 0 && index < order.length - 1) {
                const random = seededRandom(hashSeed(`${fixture.id}|next`));
                current.fixtures.push(makeFixture(
                    current.competitionId === 'EURO' ? TOURNAMENTS.EURO : TOURNAMENTS.WORLD_CUP,
                    current.seasonYear,
                    current.nationalTeam,
