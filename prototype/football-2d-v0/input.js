export function createInputController({joystick,stick,shoot,powerFill}){
  ...
  function setJoy(event){const samples=typeof event.getCoalescedEvents==="function"?event.getCoalescedEvents():null;const e=samples?.length?samples[samples.length-1]:event;const r=joystick.getBoundingClientRect();let x=(e.clientX-r.left-r.width/2)/(r.width*.35);let y=(e.clientY-r.top-r.height/2)/(r.height*.35);const length=Math.hypot(x,y);if(length>1){x/=length;y/=length;}const magnitude=Math.hypot(x,y);if(magnitude<.04){x=0;y=0;}joy.x=x;joy.y=y;stick.style.transform=`translate(calc(-50% + ${x*28}px),calc(-50% + ${y*28}px))`;}
  ...
  joystick.addEventListener(moveEvent,e=>{if(e.pointerId===joy.pointer)setJoy(e);});
