...
import { createControlLabState, stepControlLab } from "./controlLabModel-v4.js";
import { renderControlLab } from "./renderer-v4.js";
...
function labelAction(action) {
  if (action === "appel") return "APPEL · PASSE";
...
function frame(now) {
  ...
  if (!restartAt && !state.possession && !state.incomingPassActive && !state.teammateHasBall && (state.lastEvent === "tackle" || state.lastEvent === "save")) restartAt = now + 1100;
  ...
  passButton.classList.toggle("unavailable", !state.possession && state.incomingPassActive);
  if (state.possession) passButton.textContent = "PASSE";
  else if (state.incomingPassActive) passButton.textContent = "…";
  else passButton.textContent = "APPEL";
