...
export function createControlLabInput({
  ...
  function protectOn(event) {
    event.preventDefault();
    protectHeld = true;
    protect.setPointerCapture?.(event.pointerId);
  }
  function protectOff(event) {
    event.preventDefault();
    protectHeld = false;
  }
  protect.addEventListener("pointerdown", protectOn);
  protect.addEventListener("pointerup", protectOff);
  protect.addEventListener("pointercancel", protectOff);
  ...
  return {
    read() {
      ...
      return {
        ...
        controlX,
        controlY,
        protecting: protectHeld || keys.has("shift"),
        passPressed,
