import {
  ...
  actionLabels, controlMode, stepGameplay, cameraGeometry, cameraFromBall, isPointVisible,
...
function clipVision(context, player) {
...
function render() {
  ...
  ctx.save();
  ctx.globalAlpha = RULES.blindPitchAlpha;
  drawPitch(ctx);
  ctx.restore();

  ctx.save();
  clipVision(ctx, player);
  drawPitch(ctx);
  for (const other of state.players) if (!other.controlled) drawPlayer(ctx, other);
  drawPassLock(ctx);
  drawBall(ctx);
  ctx.restore();
  ...
  ctx.fillText(`STP 11v11 · ${state.score.home}-${state.score.away} · ${mode} · DOM ${homePhase} / EXT ${awayPhase} · ${state.lastEvent} · Z ${settings.zoom.toFixed(2)}`, 28, VIEWPORT.height - 28);
  ctx.restore();
}
...
function syncHud() {
  ...
  hudDetail.textContent = `${modeText(mode)} · Vision ${RULES.visionDegrees}° · Bloc ${state.tactical.home?.phase ?? "-"}`;
}
