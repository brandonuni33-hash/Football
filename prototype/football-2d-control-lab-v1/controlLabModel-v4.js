...
function createStartingSetup(base) {
  const teammate = { x: 145, y: FIELD.h / 2 + 76 };
  return {
    ...base,
    possession: false,
    teammate,
    ...
    ball: { x: teammate.x + 24, y: teammate.y - 5, vx: 0, vy: 0 },
    lastEvent: "waiting_for_call",
  };
}

export function createControlLabState() {
  const base = createStartingSetup(createScenarioState());
  return {
    ...base,
    controlMode: "vision",
    lookX: 1,
...
function requestTeammatePass(state) {
  if (state.possession || state.incomingPassActive || !state.teammateHasBall || !state.teammate) {
    return { state, requested: false };
  }
  const direction = normalize(state.player.x - state.teammate.x, state.player.y - state.teammate.y);
  return {
    requested: true,
    state: {
      ...state,
      ...
      ball: {
        x: state.teammate.x + direction.x * 24,
        y: state.teammate.y + direction.y * 24,
        vx: direction.x * CONTROL_RULES.requestedPassSpeed,
        vy: direction.y * CONTROL_RULES.requestedPassSpeed,
      },
      lastEvent: "requested_pass",
      lastControlAction: "appel",
    },
  };
}
...
function protectionContextAvailable(state) {
  return !!state.possession || !!state.incomingPassActive || !!state.teammateHasBall;
}
...
export function stepControlLab(
  ...
  const passRequest = input.passPressed ? requestTeammatePass(state) : { state, requested: false };
  const planted = applyFootPlant(passRequest.state, input, time);
  ...
  if (!passRequest.requested && input.passPressed && next.possession) next = { ...next, lastControlAction: "passe" };
  else if (passRequest.requested) next = { ...next, lastControlAction: "appel" };
