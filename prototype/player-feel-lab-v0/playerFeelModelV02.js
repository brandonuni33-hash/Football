...
export const FEEL_RULES = Object.freeze({
  ...
  plantDuration: 0.10,
  plantCooldownDuration: 0.28,
  plantSpeedScale: 0.50,
  bodyTurnDegreesPerSecond: 285,
  lowSpeedTurnDegreesPerSecond: 390,
  gaitStride: 20,
  gaitWidth: 11,
...
function updateMotion(player, input, dt) {
  ...
  const rate = targetSpeed > currentSpeed ? FEEL_RULES.acceleration : FEEL_RULES.deceleration;
  const nextSpeed = approach(currentSpeed, targetSpeed, rate * dt);
  ...
  if (desired.magnitude > 0.01) {
    ...
    const response = player.plantRemaining > 0 ? 0.36 : 0.40;
...
export function mannequinPose(state) {
  ...
  const pointFrom = (origin, forwardOffset, sideOffset) => ({
    x: origin.x + forward.x * forwardOffset + right.x * sideOffset,
    y: origin.y + forward.y * forwardOffset + right.y * sideOffset,
  });
  const foot = (forwardOffset, sideOffset) => pointFrom({ x: p.x, y: p.y }, forwardOffset, sideOffset);
  ...
  const shoulderWidth = 11;
  const leftShoulder = pointFrom(torso, 1, -shoulderWidth);
  const rightShoulder = pointFrom(torso, 1, shoulderWidth);
  let leftArmForward = -swing * (9 + speedRatio * 5);
  let rightArmForward = swing * (9 + speedRatio * 5);
  let leftArmSide = -16;
  let rightArmSide = 16;
  ...
  if (p.mode === "idle") {
    ...
    const sign = p.plantTurnSign || 1;
    leftArmForward = sign < 0 ? -9 : 10;
    rightArmForward = sign < 0 ? 10 : -9;
    leftArmSide = sign < 0 ? -22 : -18;
    rightArmSide = sign < 0 ? 18 : 22;
  }

  const leftElbow = pointFrom(leftShoulder, leftArmForward * 0.55, leftArmSide * 0.52);
  const rightElbow = pointFrom(rightShoulder, rightArmForward * 0.55, rightArmSide * 0.52);
  const leftHand = pointFrom(leftShoulder, leftArmForward, leftArmSide);
  const rightHand = pointFrom(rightShoulder, rightArmForward, rightArmSide);
