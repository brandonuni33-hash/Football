...
const cameraTiltInput = document.querySelector("#camera-tilt");
const cameraTiltOutput = document.querySelector("#camera-tilt-value");
const cameraYawInput = document.querySelector("#camera-yaw");
const cameraYawOutput = document.querySelector("#camera-yaw-value");
const cameraZoomInput = document.querySelector("#camera-zoom");
const cameraZoomOutput = document.querySelector("#camera-zoom-value");

let cameraTiltDeg = Number(cameraTiltInput?.value ?? 28);
let cameraYawDeg = Number(cameraYawInput?.value ?? -18);
let cameraZoom = Number(cameraZoomInput?.value ?? 1);
let state = createPlayerFeelState();
let accumulator = 0;
let previous = performance.now();

function updateCameraControls() {
  cameraTiltDeg = Number(cameraTiltInput?.value ?? 28);
  cameraYawDeg = Number(cameraYawInput?.value ?? -18);
  cameraZoom = Number(cameraZoomInput?.value ?? 1);
  if (cameraTiltOutput) cameraTiltOutput.textContent = `${cameraTiltDeg}°`;
  if (cameraYawOutput) cameraYawOutput.textContent = `${cameraYawDeg}°`;
  if (cameraZoomOutput) cameraZoomOutput.textContent = `${cameraZoom.toFixed(2)}x`;
}
for (const input of [cameraTiltInput, cameraYawInput, cameraZoomInput]) {
  input?.addEventListener("input", updateCameraControls);
}
...
function render() {
  ...
  const verticalScale = Math.cos(cameraTiltDeg * Math.PI / 180);
  ...
  hud.textContent = `V0.10 · 3/4 ${cameraYawDeg}° · inclinaison ${cameraTiltDeg}° · zoom ${cameraZoom.toFixed(2)}x · ${Math.round(p.speed)} u/s`;
}
