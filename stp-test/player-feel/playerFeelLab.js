...
function drawMannequin() {
  ...
  ctx.fillStyle = "#000";
  ctx.beginPath();
  ctx.ellipse(p.x + 5, p.y + 8, 24, 12, p.facing, 0, Math.PI * 2);
  ctx.fill();
  ctx.restore();
...
function render() {
  ...
  ctx.fillRect(16, VIEWPORT.height - 49, 740, 32);
  ctx.fillStyle = "#f3f1eb";
  ctx.font = "800 12px system-ui";
  ctx.textAlign = "left";
  ctx.textBaseline = "middle";
  ctx.fillText(
    `${p.mode.toUpperCase()} · ${Math.round(p.speed)} u/s · rotation ${Math.round(p.turnDeltaDegrees)}° · appui ${p.plantedFoot.toUpperCase()} · ${p.rapid ? "RAPIDE" : "NORMAL 76%"}`,
    28,
    VIEWPORT.height - 33,
  );
  ctx.restore();

  hud.textContent = `Déplacement · ${Math.round(p.speed)} u/s · ${p.mode} · appui ${p.plantedFoot}`;
}
