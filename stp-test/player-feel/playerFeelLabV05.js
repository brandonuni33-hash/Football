import { VIEWPORT, PITCH, FEEL_RULES, createPlayerFeelState, stepPlayerFeel, mannequinPose } from "./playerFeelModelV05.js";
...
function joint(point, radius, color) {
  ctx.fillStyle = color;
  ctx.beginPath();
...
function drawPlayer() {
  ...
  const left = pose.plantedFoot === "left";
  const right = pose.plantedFoot === "right";
  ...
  limb(pose.leftShoulder, pose.leftElbow, 7, "#d7aa84");
  limb(pose.leftElbow, pose.leftHand, 6, "#d7aa84");
  limb(pose.rightShoulder, pose.rightElbow, 7, "#d7aa84");
  limb(pose.rightElbow, pose.rightHand, 6, "#d7aa84");
  joint(pose.leftHand, 4.2, "#d7aa84");
  joint(pose.rightHand, 4.2, "#d7aa84");

  limb(pose.hip, pose.leftKnee, 9, "#e8e6df");
  limb(pose.leftKnee, pose.leftFoot, 8, "#d6d4cd");
  limb(pose.hip, pose.rightKnee, 9, "#e8e6df");
  limb(pose.rightKnee, pose.rightFoot, 8, "#d6d4cd");
  joint(pose.leftFoot, left ? 7 : 5.5, left ? "#e7af3f" : "#f0eee8");
  joint(pose.rightFoot, right ? 7 : 5.5, right ? "#e7af3f" : "#f0eee8");
  limb(pose.hip, pose.torso, 10, "#0d1218");
  ...
  if (p.mode === "plant") {
    ctx.strokeStyle = "rgba(231,175,63,.78)";
    ctx.lineWidth = 2;
    ctx.beginPath();
    ctx.arc(pose.plantAnchor?.x ?? p.x, pose.plantAnchor?.y ?? p.y, 18, 0, Math.PI * 2);
    ctx.stroke();
  }
}
...
function render() {
  ...
  const p = state.player;
  hud.textContent = `V0.5 · angle caméra ${cameraAngleDeg}° · ${Math.round(p.speed)} u/s · buste ${p.torsoLeanDegrees.toFixed(1)}° · ${p.mode}`;
}
