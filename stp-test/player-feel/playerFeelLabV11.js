...
const CAMERA_PRESETS = Object.freeze({
  broadcast: { label: "BROADCAST HAUTE", angle: 12, zoom: 0.84, follow: "field", extraLead: -22, rotation: 0 },
  mid: { label: "INTERMÉDIAIRE", angle: 26, zoom: 1.00, follow: "field", extraLead: 18, rotation: 0 },
  behind: { label: "PROCHE DERRIÈRE", angle: 38, zoom: 1.18, follow: "behind", extraLead: 118, rotation: 0 },
  threequarter: { label: "3/4 OBLIQUE", angle: 29, zoom: 1.03, follow: "field", extraLead: 34, rotation: -20 * DEG },
});
...
function render() {
  ...
  const verticalScale = Math.cos(cameraAngleDeg * DEG);
  ...
  const p = state.player;
  hud.textContent = `V0.11 · ${CAMERA_PRESETS[cameraMode].label} · ${Math.round(cameraAngleDeg)}° · ${cameraZoom.toFixed(2)}× · ${Math.round(p.speed)} u/s`;
}

function cameraRelativeInput(x, y) {
  const camera = cameraTransform();
  if (Math.abs(camera.rotation) < 0.0001) return { x, y };
  const inverseRotation = -camera.rotation;
