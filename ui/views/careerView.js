...
function searchableText(item) { return `${categoryOf(item)} ${item?.title || ''} ${item?.label || ''}`.toLowerCase(); }
...
function isMercatoItem(item) { return /transfer|transfert|mercato|scout|recruteur|observateur|offre|proposition de club/.test(searchableText(item)); }
function journalKey(item) { return `${String(item?.title || '').trim().toLowerCase()}|${String(item?.text || item?.body || item?.message || item?.description || '').trim().toLowerCase()}`; }
function dedupe(items = []) { const seen=new Set(); return items.filter(item=>{const key=journalKey(item);if(!key||seen.has(key))return false;seen.add(key);return true;}); }
...
export class CareerView {
    ...
    render(state) {
        ...
        // Le journal raconte la carrière sportive et personnelle. Le mercato reste dans son écran dédié.
        const signals=dedupe((Array.isArray(notificationState)?notificationState:(notificationState?.signals||[]))
            .filter(note=>!note?.archived&&!isSocialNotification(note)&&!isDedicatedSection(note)&&!isMercatoItem(note))).slice(-30);
        const unreadCount=signals.filter(note=>!note?.read).length;
        const narrativeEntries=dedupe((this.narrativePresenter?.getJournal?.(state)||[])
            .filter(entry=>!isMercatoItem(entry))).slice(-20);
        return `
