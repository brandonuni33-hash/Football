...
export class CreationController {
    ...
    render() {
        ...
        return `<div class="career-container"><header class="career-header"><h1>⚡ Street to Pro</h1><div class="progress-bar"><div class="progress" style="width:${(currentStep / 5) * 100}%"></div></div></header><main class="career-content">${this.renderStep(currentStep, selectedData)}</main><footer class="career-footer">${currentStep > 1 ? '<button id="prev-btn" class="btn-secondary" type="button">Précédent</button>' : ''}${currentStep < 5 ? '<button id="next-btn" class="btn-primary" type="button" disabled>Suivant</button>' : '<button id="start-btn" class="btn-success" type="button" disabled>Lancer</button>'}</footer></div>`;
    }
    renderStep(step, data) {
        switch (step) {
            case 1: return `<h2>Étape 1 : Identité & Poste</h2><div class="form-group"><label for="firstname">Prénom :</label><input id="firstname" type="text" value="${escape(data.firstname)}" placeholder="ex: Kylian"></div><div class="form-group"><label for="lastname">Nom :</label><input id="lastname" type="text" value="${escape(data.lastname)}" placeholder="ex: Mbappé"></div><div class="form-group"><label>Choisis ton poste sur le terrain :</label><div class="proclubs-pitch-container"><div class="proclubs-soccer-pitch">${positions.map(position => { const id = position?.id; if (!id) return ''; const coords = this.positionCoords(id); return `<button type="button" class="proclubs-node ${data.position === id ? 'selected' : ''}" data-pos="${escape(id)}" style="top:${coords.top};left:${coords.left}" title="${escape(position?.name || id)}"><div class="proclubs-jersey">👕</div><span class="proclubs-pos-name">${escape(id)}</span></button>`; }).join('')}</div></div></div>`;
