...
export function installCoreStyles() {
    ...
    style.textContent = `
        ...
        .event-modal-overlay{position:absolute;inset:0;background:rgba(2,6,23,.88);backdrop-filter:blur(20px);z-index:100;display:flex;align-items:flex-start;justify-content:center;padding:20px;overflow-y:auto;-webkit-overflow-scrolling:touch;overscroll-behavior:contain;touch-action:pan-y;scroll-behavior:smooth}
        .event-modal-card{background:#1e293b;border:1.5px solid var(--border-glass);border-radius:26px;padding:22px;width:100%;max-width:360px;max-height:calc(100dvh - 40px);overflow-y:auto;overflow-x:hidden;-webkit-overflow-scrolling:touch;overscroll-behavior:contain;touch-action:pan-y;box-shadow:0 20px 40px rgba(0,0,0,.8);margin:auto 0;scrollbar-width:thin}
        .event-modal-category{font-size:.72rem;font-weight:800;letter-spacing:1px;color:var(--accent-gold);text-transform:uppercase}.event-modal-title{font-size:1.25rem;font-weight:800;margin:6px 0 10px;color:#fff}.event-modal-desc{font-size:.9rem;color:var(--text-sub);line-height:1.4;margin-bottom:18px}.btn-event-choice{width:100%;padding:14px;border-radius:14px;background:rgba(255,255,255,.1);border:1px solid var(--border-glass);color:var(--text-main);font-weight:700;font-size:.9rem;margin-bottom:10px;cursor:pointer;text-align:left}
        @media (max-width:480px){.event-modal-overlay{padding:12px}.event-modal-card{max-height:calc(100dvh - 24px);border-radius:22px}}
    `;
