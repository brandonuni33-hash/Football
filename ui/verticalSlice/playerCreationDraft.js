...
export const CHILDHOOD_CONTINENTS = Object.freeze(['Europe', 'Afrique']);
...
export function createPlayerCreationDraft(seed = {}) {
    return {
        ...
        raisedInCountry: seed.raisedInCountry ?? null,
        raisedInContinent: CHILDHOOD_CONTINENTS.includes(seed.raisedInContinent) ? seed.raisedInContinent : null,
...
export function validatePlayerCreationDraft(draft = {}) {
    ...
    if (!Number.isFinite(height) || height < VERTICAL_SLICE_BODY_LIMITS.height.min || height > VERTICAL_SLICE_BODY_LIMITS.height.max) errors.height = 'Taille invalide.';
    if (!Number.isFinite(weight) || weight < VERTICAL_SLICE_BODY_LIMITS.weight.min || weight > VERTICAL_SLICE_BODY_LIMITS.weight.max) errors.weight = 'Poids invalide.';
    ...
    if (draft.secondaryNationality && draft.secondaryNationality === draft.primaryNationality) errors.secondaryNationality = 'La seconde nationalité doit être différente.';
    if (!CHILDHOOD_CONTINENTS.includes(draft.raisedInContinent)) errors.raisedInContinent = "Continent d'enfance requis.";
...
export function toCareerCreationIdentity(draft = {}) {
    ...
    return Object.freeze({
        firstname: String(draft.firstname).trim(), lastname: String(draft.lastname).trim(), age: VERTICAL_SLICE_START_AGE,
        faceId: draft.faceId, height: Number(draft.height), weight: Number(draft.weight), position: draft.position,
        preferredFoot: draft.preferredFoot, nationality: draft.primaryNationality, primaryNationality: draft.primaryNationality,
        secondaryNationality: draft.secondaryNationality || null, raisedInCountry: null, raisedInContinent: draft.raisedInContinent,
