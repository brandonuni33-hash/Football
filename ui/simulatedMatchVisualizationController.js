...
function pitchHtml(tactical,playerSide){
    ...
    return `<div class="sim-match-pitch" data-simulated-pitch data-own-formation="${esc(tactical.formations.own)}" data-opponent-formation="${esc(tactical.formations.opponent)}" data-player-position="${esc(tactical.playerFocal?.position||'')}">
...
export class SimulatedMatchVisualizationController{
    constructor(){this.timeline=null;this.index=0;this.overlay=null;this.textTimer=null;this.textComplete=true;this.onComplete=null;this.playerPosition='BU';}
    ...
    show(match,{player={},seed=null,onComplete=null}={}){
        ...
        overlay.className='simulated-match-overlay';overlay.dataset.simulatedMatchVisualization='true';overlay.dataset.playerPosition=this.playerPosition;
    ...
    renderCurrent(){
        ...
        const stage=this.overlay.querySelector('[data-sim-stage]');if(stage)stage.innerHTML=pitchHtml(tactical,this.timeline.playerSide);
        this.overlay.dataset.eventType=event.type;this.overlay.dataset.cameraState=event.cameraState;this.overlay.dataset.tacticalLevel=tactical.organization.level;this.overlay.dataset.playerSide=this.timeline.playerSide;this.overlay.dataset.ownFormation=tactical.formations.own;this.overlay.dataset.opponentFormation=tactical.formations.opponent;
