...
function actorHtml(point,index,team,{playerSide='HOME',focal=null}={}){
    ...
    const own=team.toUpperCase()===playerSide;
    const isFocal=Boolean(focal&&focal.team===team.toUpperCase()&&Number(focal.index)===index);
...
function pitchHtml(tactical,playerSide){
    const options={playerSide,focal:tactical.playerFocal};
    const home=tactical.home.map((point,index)=>actorHtml(point,index,'home',options)).join('');
    const away=tactical.away.map((point,index)=>actorHtml(point,index,'away',options)).join('');
    return `<div class="sim-match-pitch" data-simulated-pitch data-own-formation="${esc(tactical.formations.own)}" data-opponent-formation="${esc(tactical.formations.opponent)}">
...
export class SimulatedMatchVisualizationController{
    constructor(){this.timeline=null;this.index=0;this.overlay=null;this.textTimer=null;this.textComplete=true;this.onComplete=null;}
    ...
    renderCurrent(){
        ...
        const tactical=buildSimulatedMatchTacticalSituation(event,{
            playerAge:this.timeline.playerAge,competition:this.timeline.competition,
            seed:this.timeline.matchId,playerSide:this.timeline.playerSide
        });
        const stage=this.overlay.querySelector('[data-sim-stage]');
        if(stage)stage.innerHTML=pitchHtml(tactical,this.timeline.playerSide);
        ...
        this.overlay.dataset.playerSide=this.timeline.playerSide;
        this.overlay.dataset.ownFormation=tactical.formations.own;
        this.overlay.dataset.opponentFormation=tactical.formations.opponent;
    ...
    titleFor(event){
        const titles={KICKOFF:"Coup d'envoi",BUILD_UP:'La construction prend forme',PRESSING:'Le pressing resserre le jeu',DUEL:'Le duel se rapproche',COUNTER_ATTACK:'La transition accélère',CROSS:"Le jeu s'ouvre sur un côté",SHOT:'Une fenêtre de frappe',SET_PIECE:'Coup de pied arrêté',GOAL:'Le score change',FULL_TIME:'Coup de sifflet final'};
        return titles[event.type]||'Le match continue';
    }
    revealProgressively(text){
        this.clearTextTimer();const node=this.overlay?.querySelector('[data-sim-text]');if(!node)return;
        const reduced=window.matchMedia?.('(prefers-reduced-motion: reduce)')?.matches===true,parts=String(text).split(/(\s+)/).filter(Boolean);
        if(reduced||parts.length<=2){node.textContent=text;node.dataset.textComplete='true';this.textComplete=true;return;}
        node.textContent='';node.dataset.textComplete='false';this.textComplete=false;let cursor=0;
        this.textTimer=window.setInterval(()=>{node.textContent+=parts[cursor++]||'';if(cursor>=parts.length){this.clearTextTimer();node.dataset.textComplete='true';this.textComplete=true;}},28);
    }
