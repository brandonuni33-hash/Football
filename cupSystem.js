// cupSystem.js
// Propriétaire canonique des coupes nationales.
// Tirage, tours, qualification, finale, pénaltys et historique.
...
function resolve(home, away, random = Math.random, forced = null) {
    ...
    if (homeGoals == null || awayGoals == null) {
        ({ homeGoals, awayGoals } = simulateGoals(home, away, random));
    }
...
function match(home, away, cup, round, month, index) {
    ...
    return {
        ...
        homeClub: h.name,
        awayClub: a.name,
        opponent: h.id === p ? a.name : h.name,
        venue: h.id === p ? 'Domicile' : 'Extérieur',
...
function buildRound(cup, ids) {
    ...
    if (clubs.length <= 1) {
        return { matches: [], byes: clubs.map(c => c.id), round: 'Finale', month: 5 };
    }
    ...
    if (clubs.length > target) {
        ...
        byes = ordered.slice(0, clubs.length - matchClubCount).map(c => c.id);
    ...
    for (let i = 0; i < drawPool.length; i += 2) {
        matches.push(match(drawPool[i], drawPool[i + 1], cup, plan.round, plan.month, i / 2));
    }
...
export const CupSystem = {
    ...
    getPlayerMatch(state, country = countryForPlayer(state)) {
        ...
        if (!cup.matches.length && (!Number(cup.roundMonth) || month >= Number(cup.roundMonth))) {
            this.prepareRound(state, country);
        }
    ...
    getSummary(state, country = countryForPlayer(state)) {
        ...
        return {
            ...
            playerStillIn: cup.status !== 'finished' && cup.qualifiedClubIds.includes(pid),
