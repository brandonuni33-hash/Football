...
describe('goalOpportunityChoiceLibrary', () => {
  it('contains the 20 authored goal opportunities with exactly three choices each', () => {
    expect(GOAL_OPPORTUNITY_CHOICES).toHaveLength(20);
    expect(new Set(GOAL_OPPORTUNITY_CHOICES.map(item => item.id)).size).toBe(20);
  ...
  it('keeps the authored scenario text when building the playable decision', () => {
    ...
    expect(decision.opportunityId).toBe('OCC-005');
    expect(decision.title).toContain("63'");
    expect(decision.description).toContain('un seul défenseur');
