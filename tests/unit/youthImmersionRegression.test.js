...
test('une micro-scène U15 ne cite plus le nom du club adverse', () => {
    ...
    const text = decisionNarration({
        minute:34, index:0, position:'attaquant', age:15,
        level:'Championnat National U15', competition:'Championnat National U15',
        opponent:'FC Martigues U15', score:{home:0,away:0}, home:true,
        confidence:55, fatigue:0, coachTrust:50, directOpponent
    });
...
test('l’origine futsal ouvre un choix sans s’afficher comme un trait', () => {
    const state = {
        player:{ id:'p-futsal', age:15, club:'Trélissac FC', position:'BU', overall:70, origin:'FUTSAL',
            attributes:{controle:78,dribble:78,tir:70,passe:66,puissance:60,vitesse:72}, stats:{} },
        social:{coachData:{name:'Coach'}}, career:{balance:0}, consequences:[], careerMemory:[]
    };
    const fixture = { id:'futsal-u15', competitionName:'Championnat National U15', opponent:'FC Martigues U15', home:true, opponentStrength:55, playerSelection:{selected:true,started:true,minutes:70} };
    const session = startInteractiveMatch(state, fixture, 0);
    advanceInteractiveMatch(state, session); // kickoff
    const output = advanceInteractiveMatch(state, session); // first decision
...
test('les médias restent coupés en U15 même après plusieurs matchs', () => {
    ...
    const state = {
        player:{ id:'u15-media', firstname:'Alex', lastname:'Test', age:15, squadStatus:'U15', careerStage:'youth', stats:{matchesPlayed:12} },
        calendar:{currentSeason:1,currentMonth:4},
