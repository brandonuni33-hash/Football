...
test("double-push burst is short and exposes the ball farther",()=>{let state=createScenarioState();state=stepScenario(state,{moveX:1,burstTriggered:true,burstX:1,burstY:0},1/60,DEFAULT_FEEL_TUNING,{speed:80,acceleration:80});assert.ok(state.burstTime>0);assert.equal(state.dribbleMode,"poussee");for(let i=0;i<30;i+=1)state=stepScenario(state,{moveX:1},1/60,DEFAULT_FEEL_TUNING,{speed:80,acceleration:80});assert.equal(state.burstTime,0);});
