...
const CompetitionSystem = {
    ...
    finalizeSchedule(player, seasonYear, matches, category, seed) { const ordered = sortMatches(matches), byMonth = {}; for (const month of ALL_MONTHS) byMonth[month] = { month, label: this.getMonthLabel(month), period: this.getPeriodName(month), phase: MONTH_INFO[month]?.phase || 'offseason', matches: [] }; ordered.forEach(match => byMonth[match.month].matches.push(match)); return { version: 4, seasonYear, seasonLabel: seasonLabel(seasonYear), generatedForAge: Number(player?.age) || 14, category, seed, matches: ordered, byMonth, totals: { allMatches: ordered.length, leagueMatches: ordered.filter(match => match.type === 'league').length, cupMatches: ordered.filter(match => match.competitionId === 'NATIONAL_CUP').length, europeanMatches: ordered.filter(match => ['CHAMPIONS_LEAGUE', 'EUROPA_LEAGUE'].includes(match.competitionId)).length } }; },
