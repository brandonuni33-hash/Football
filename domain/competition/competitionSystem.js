...
const CompetitionSystem = {
    ...
    createSeasonSchedule(player, seasonYear) {
        const generated = buildSeasonSchedule(player, seasonYear, this.getSeniorCompetition.bind(this), buildEuropeanLeague, getEuropeanQualification);
    ...
    finalizeSchedule(player, seasonYear, matches, category, seed) {
        ...
        const byMonth = {};
        for (const month of ALL_MONTHS) byMonth[month] = { month, label: this.getMonthLabel(month), period: this.getPeriodName(month), phase: MONTH_INFO[month]?.phase || 'offseason', matches: [] };
        ordered.forEach(match => byMonth[match.month].matches.push(match));
        return {
            version: 4, seasonYear, seasonLabel: seasonLabel(seasonYear), generatedForAge: Number(player?.age) || 14,
            category, seed, matches: ordered, byMonth,
            totals: {
    ...
    ensureSeasonSchedule(state) {
        ...
        state.europeanTournament = Number(state.player.age) >= 18 ? createEuropeanTournament(state.player, year, qualification, schedule.seed) : null;
        return schedule;
    },
    ...
    getBlockPlan(state) {
        const player = state?.player || {}, calendar = state?.calendar || {}, month = Number(calendar.currentMonth) || 8, seasonYear = Number(calendar.currentSeasonYear) || new Date().getFullYear();
        const schedule = this.ensureSeasonSchedule(state), monthData = schedule?.byMonth?.[month];
        if (this.isOffSeason(month)) return {
            type: 'offseason', month, monthLabel: this.getMonthLabel(month), season: seasonYear, seasonLabel: seasonLabel(seasonYear), matches: 0, scheduledMatches: [],
            activities: month === 7 ? ['repos', 'mercato', 'programme_individuel', 'preparation_saison'] : ['bilan', 'selection_internationale', 'repos', 'recovery'], importance: 'none', mode: 'career_activity'
        };
        ...
        const hasMajor = base.some(match => match.importance === 'major'), hasImportant = base.some(match => match.importance === 'important');
        return {
            type: player.age < 18 ? 'youth' : 'senior', category: schedule?.category || (player.age < 18 ? this.getYouthCategory(player.age) : 'Senior'),
            month, monthLabel: this.getMonthLabel(month), season: seasonYear, seasonLabel: seasonLabel(seasonYear), matches: base.length, scheduledMatches: base,
            competition: base[0]?.competitionName || null, activities: base.length ? [player.age < 18 ? 'match_jeunes' : 'match', 'entrainement'] : ['entrainement', 'evenement'],
            importance: hasMajor ? 'major' : hasImportant ? 'important' : base.length >= 4 ? 'normal' : 'low', mode: hasMajor ? 'major' : hasImportant ? 'mixed' : base.length ? 'simulation' : 'career_activity'
        };
    },
    ...
    recordEuropeanResults(state, scheduledMatches, matchResults) {
        ...
        for (const match of european) {
            ...
            if (match.phase === 'league_phase') {
                ...
                if (opponentRow) { opponentRow.played += 1; opponentRow.points += Math.max(0, 3 - performancePoints); opponentRow.goalsAgainst += goals; opponentRow.goalsFor += rating < 6 ? 2 : 1; }
                const ownRow = tournament.standings.find(row => row.clubId === match.playerClubId);
                if (ownRow) { ownRow.played += 1; ownRow.points = tournament.leaguePoints; ownRow.goalsFor = tournament.leagueGoalsFor; ownRow.goalsAgainst = tournament.leagueGoalsAgainst; }
    ...
    getSeasonSkeleton(player, year) {
        ...
        return ALL_MONTHS.map(month => ({ month, monthLabel: this.getMonthLabel(month), period: this.getPeriodName(month), phase: MONTH_INFO[month]?.phase || 'offseason', matches: schedule.byMonth[month]?.matches || [] }));
    }
};
