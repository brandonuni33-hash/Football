...
function distributeExact(total, weights, random) {
    ...
    const raw = SEASON_MONTHS
        .map(month => ({ month, weight: Math.max(0.001, Number(weights[month] || 0.1)) }))
        .map(item => ({ ...item, exact: total * item.weight }));
    let assigned = 0;
    raw.forEach(item => {
...
function buildYouthSchedule(player, seasonYear, random) {
    ...
    const monthly = distributeExact(total, {
        8: .09, 9: .10, 10: .11, 11: .11, 12: .08,
        1: .09, 2: .11, 3: .11, 4: .11, 5: .09
    }, random);
    const matches = [];
...
function buildSeniorSchedule(player, seasonYear, random, competition, buildEuropeanLeague, getEuropeanQualification) {
    const monthly = distributeExact(competition.matches, {
    ...
    for (const month of SEASON_MONTHS) {
        for (let i = 0; i < monthly[month]; i += 1) {
            const opponent = opponents.length ? opponents[cursor++ % opponents.length] : null;
    ...
    const extras = buildEuropeanLeague(player, seasonYear, random, qualification);
...
export function createSeasonSchedule(player, seasonYear, getSeniorCompetition, buildEuropeanLeague, getEuropeanQualification) {
    ...
    const senior = buildSeniorSchedule(player, seasonYear, random, competition, buildEuropeanLeague, getEuropeanQualification);
