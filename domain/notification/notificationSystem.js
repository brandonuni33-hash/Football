...
export class NotificationSystem {
    ...
    init() {
        ...
        this.#on(EVENTS.RELATIONSHIP_ADVICE, payload => this.#signal(payload, { category: 'relation', priority: 'toast', title: 'Quelqu’un souhaite vous parler', body: 'Une personne de votre entourage vous a laissé un conseil.', intent: 'conversation' }));
        this.#on(EVENTS.MEDIA_POST_CREATED, payload => this.#signal(payload, { category: 'media', priority: 'feed', title: 'Une nouvelle publication parle de vous', body: 'Une nouvelle réaction médiatique vient d’apparaître.', intent: 'media' }));
        this.started = true;
    }
    ...
    #append(signal, payload) {
        ...
        if (!thread) {
            thread = { id: signal.threadId, category: signal.category, title: signal.title, createdAt: signal.createdAt, updatedAt: signal.createdAt, signalIds: [] };
