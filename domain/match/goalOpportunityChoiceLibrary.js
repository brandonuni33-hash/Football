// Street to Pro — occasions de but jouables.
// Chaque scénario est une vraie décision de finition, avec trois approches distinctes.
// Les descriptions restent indépendantes du niveau de carrière : le contexte public/stade est injecté ailleurs.
...
export function selectGoalOpportunity({ seed, index = 0, minute = 0, position = '', scoreFor = 0, scoreAgainst = 0, fatigue = 0, directOpponent = null } = {}) {
  ...
  const candidates = GOAL_OPPORTUNITY_CHOICES.filter(item => {
  ...
  const weighted = [];
  for (const item of candidates) for (let i = 0; i < Number(item.weight || 1); i += 1) weighted.push(item);
...
export default { GOAL_OPPORTUNITY_CHOICES, selectGoalOpportunity, buildGoalOpportunityDecision, goalOpportunityPositionGroup };
