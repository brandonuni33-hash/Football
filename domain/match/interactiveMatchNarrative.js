...
import { resolveInteractiveMatchPresentation } from './interactiveMatchPresentationState.js';
...
function step(session, values = {}) {
    const presentation = resolveInteractiveMatchPresentation(values);
    return {
...
function consequenceInnerVoice(session,event,index){
    ...
    return innerVoicePick(session,`consequence:${index}:${failed?'fail':'success'}`,item=>failed?(item.family==='doubt'||item.tags?.includes('after_mistake')):(item.family==='confidence'||item.family==='direct_opponent'||item.tags?.includes('confidence_high')));
}
...
export function buildConsequenceStep(session,event,index){const source=`${event?.title||''} ${event?.text||''}`,failed=/échoue|ne passe pas|perd le ballon|récupère|fenêtre se referme|lit ton geste/i.test(source),duel=/duel|vis-à-vis|adversaire direct/i.test(source);let text=event?.text||'';if(event?.timedOut||event?.gesture||duel){text=decisionOutcomeText({gesture:event?.gesture||null,choice:event?.choice||null,success:!failed,duel,timedOut:Boolean(event?.timedOut),minute:event?.minute,score:scoreSnapshot(session),home:session.home,position:positionNarrativeLabel(session.playerPosition||session.match?.playerPosition),confidence:confidenceValue(session)});}else if(!text||genericControllerCopy(text)){text=stablePick(matchSeed(session),`consequence:${index}:${event?.minute||0}`,failed?MATCH_COPY.failure:MATCH_COPY.success);}return step(session,{phase:`consequence_${index+1}`,label:'CONSÉQUENCE',progress:Math.min(86,28+index*13),minute:event.minute,title:event.title||(failed?'L’action se referme':'Tu gagnes un temps'),text,innerVoice:consequenceInnerVoice(session,event,index),actionLabel:'Continuer'});}
