...
const SAFE_CHOICES=[
    {text:'Prendre une seconde pour lire le prochain déplacement',impacts:{stats:{mental:1},matchBonuses:{ratingBonus:.05,fatigueRisk:-1}}},
    {text:'Jouer simple pour remettre l’équipe dans le bon tempo',impacts:{stats:{discipline:1},matchBonuses:{passAccuracy:.08,ratingBonus:.05}}},
    {text:'Hausser l’intensité sur la prochaine action',impacts:{stats:{physique:1},matchBonuses:{duelBonus:.05,fatigueRisk:1}}},
    {text:'Chercher le partenaire le mieux placé avant de forcer',impacts:{stats:{mental:1,technique:1},matchBonuses:{teamBoost:.05,assistChance:.025}}}
];
function ensureFour(dilemma){
    const seen=new Set((dilemma?.choices||[]).map(choice=>choice.text));
    const choices=[...(dilemma?.choices||[])];
    for(const choice of SAFE_CHOICES){if(choices.length>=4)break;if(!seen.has(choice.text)){seen.add(choice.text);choices.push(choice);}}
...
export class MatchChoiceManager {
    ...
    static getMatchDilemma(matchType,opponentName,position='BU'){
        ...
        if(matchType==='rival')return ensureFour({type:'rival',title:'🔥 Le derby contre '+opponentName,description:"Les duels montent en intensité et le public réagit à chaque contact. Il faut choisir votre manière d’entrer dans ce rapport de force.",choices:[
        ...
        return ensureFour(pick(roleDilemmas[group(position)]||roleDilemmas.attacker));
    }
}
