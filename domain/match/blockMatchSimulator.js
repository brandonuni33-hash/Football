...
export function simulateBlock(state, trainingFocus = 'TECHNIQUE', userMatchChoice = null) {
    ...
    const results = scheduledMatches.map((scheduledMatch, matchIndex) => {
        ...
        return buildMatchResult({
            ...
            rating,
            group,
            goalChance: clamp(.04 + attackingFactor * .16 + goalBonusChance, .01, .75),
            assistChance: clamp(.06 + number(player.attributes?.passe ?? 40) / 99 * .18 + assistBonusChance, .01, .75),
    ...
    const yellowCards = matchesInBlock
        ? results.reduce(sum => sum, 0) + results.reduce(sum => sum + (Math.random() < clamp(.04 + choiceCardRiskExtra, 0, .6) ? 1 : 0), 0)
        : 0;
