...
const clubProfile = club => {
    if (!club) return null;
    const name = club.name || club.nom;
    if (!name) return null;
    const prestigeRaw = Number(club.prestige ?? club.reputation ?? club.reputationClub ?? 3);
    const prestige = prestigeRaw > 5 ? prestigeRaw / 20 : prestigeRaw;
    const strength = Number(club.strength ?? club.overall ?? club.minOvr ?? 65);
    return {
        id: club.id || null,
        nom: name,
        pays: club.country || club.pays || 'France',
        reputation: Math.max(1, Math.min(5, prestige || 3)),
        minOvr: Math.max(45, Math.round(strength - 4))
    };
};

export const TransferMarket = {
    calculateMarketValue(player) {
        const ovr = player.overall || 50, age = player.age || 18, poste = player.position || 'MC', nationalite = player.nationality || player.country || 'France';
        ...
        const attaquants = ['BU', 'AT', 'AG', 'AD'], milieuxOff = ['MOC', 'MG', 'MD'], milieuxDef = ['MC', 'MDC', 'MDEF'], defenseurs = ['DC', 'DG', 'DD'];
        let positionMultiplier = 1;
        if (attaquants.includes(poste)) positionMultiplier = 1.25; else if (milieuxOff.includes(poste)) positionMultiplier = 1.15; else if (milieuxDef.includes(poste)) positionMultiplier = 1; else if (defenseurs.includes(poste)) positionMultiplier = .85; else if (poste === 'G' || poste === 'GK') positionMultiplier = .7;
    ...
    generateTransferOffer(player, targetClub = null) {
        const playerOvr = player.overall || 65, marketValue = this.calculateMarketValue(player);
        const provided = clubProfile(targetClub);
        const candidates = provided
            ? [provided]
            : clubsDB.filter(club => playerOvr >= club.minOvr - 3 && playerOvr <= club.minOvr + 5);
        if (!candidates.length) return null;

        const clubAcheteur = candidates[Math.floor(Math.random() * candidates.length)];
        ...
        return {
            ...
            clubId: clubAcheteur.id || targetClub?.id || null,
            ...
            rolePropose: role,
            type: Number(player.age) < 18 ? 'recrutement_formation' : 'transfert_senior',
