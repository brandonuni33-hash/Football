...
// Les dépendances historiques sont injectées afin de permettre une migration progressive sans dupliquer les règles du calendrier.

export class CalendarSystem {
    constructor({ worldSystem, competitionSystem, cupSystem, seasonReset = null, familySystem = null } = {}) {
    ...
    advance(state) {
        ...
        if (calendar.currentMonth === 8) {
            ...
            this.worldSystem?.resetSeasonTables?.(state, calendar.currentSeasonYear);
            this.familySystem?.advanceSeason?.(state, calendar.currentSeasonYear);
            seasonChanged = true;
        }
