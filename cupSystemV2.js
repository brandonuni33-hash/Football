...
function match(home, away, cup, round, month, index) {
    ...
    return {
        id: `cup-${cup.seasonYear}-${cup.id}-${cup.roundIndex}-${index}-${h.id}-${a.id}`,
        type: 'cup', competitionType: 'national_cup', competitionId: cup.id, competitionName: cup.name,
        round, month, seasonYear: cup.seasonYear, matchday: index + 1,
        homeClubId: h.id, awayClubId: a.id, homeClub: h.name, awayClub: a.name,
        opponent: h.id === p ? a.name : h.name, venue: h.id === p ? 'Domicile' : 'Extérieur',
...
function buildRound(cup, ids) {
    ...
    let planIndex;
    if (clubs.length > 32) planIndex = 0;
    else if (clubs.length === 32) planIndex = 1;
    else if (clubs.length === 16) planIndex = 2;
    else if (clubs.length === 8) planIndex = 3;
    else if (clubs.length === 4) planIndex = 4;
    else planIndex = 5;
    ...
    if (clubs.length > target) {
        ...
        byes.push(...ordered.slice(0, clubs.length - matchClubCount).map(c => c.id));
...
export const CupSystem = {
    COUNTRIES,
        ...
        for (const country of Object.keys(COUNTRIES)) {
            ...
            const old = state.cups[def.id];
            if (!old || Number(old.seasonYear) !== year) state.cups[def.id] = {
                id: def.id, name: def.name, shortName: def.shortName, country, seasonYear: year, status: 'active',
                roundIndex: 0, currentRound: null, roundMonth: null, qualifiedClubIds: clubs.map(c => c.id),
                eliminatedClubIds: [], matches: [], history: [], finalistIds: [], winnerId: null, champion: null,
    ...
    prepareRound(state, country = countryForPlayer(state)) {
        ...
        cup.currentRound = built.round; cup.roundMonth = built.month; cup.matches = built.matches;
        cup.byeClubIds = built.byes; cup.pendingWinnerIds = [...built.byes];
        return cup.matches;
    },
    ...
    getPlayerFixtures(state) {
        const country = countryForPlayer(state), month = Number(state.calendar?.currentMonth) || 8;
        const cup = this.getCup(state, country);
        if (!cup || cup.status === 'finished') return [];
        const m = this.getPlayerMatch(state, country);
        return m && Number(m.month) === month ? [m] : [];
    },
    resolvePlayerMatch(state, fixture, performance = {}) {
        ...
        const home = WorldSystem.getClub(fixture.homeClubId), away = WorldSystem.getClub(fixture.awayClubId);
        if (!cup || !home || !away) return null;
        const pid = playerClubId(state), homePlayer = home.id === pid;
        const rating = Number(performance.rating || 6), goals = Math.max(0, Number(performance.goals || 0)), assists = Math.max(0, Number(performance.assists || 0));
        const base = simulateGoals(home, away);
        let playerScore = homePlayer ? base.homeGoals : base.awayGoals;
        playerScore = Math.min(5, Math.max(0, Math.round(playerScore + clamp((rating - 6) * .22 + assists * .08, -.5, 1.1)), Math.min(3, goals))));
        ...
        this._record(cup, fixture, result); this._finishRoundIfReady(state, cup); return result;
    },
    simulateCurrentRound(state, country = countryForPlayer(state)) {
        ...
        for (const fixture of [...cup.matches]) {
            if (fixture.played || fixture.homeClubId === pid || fixture.awayClubId === pid) continue;
            const home = WorldSystem.getClub(fixture.homeClubId), away = WorldSystem.getClub(fixture.awayClubId);
            if (home && away) this._record(cup, fixture, resolve(home, away));
        }
        this._finishRoundIfReady(state, cup); return cup;
    },
    _record(cup, fixture, result) {
        fixture.played = true; fixture.homeGoals = result.homeGoals; fixture.awayGoals = result.awayGoals; fixture.winnerClubId = result.winnerClubId;
        fixture.wentToExtraTime = result.wentToExtraTime; fixture.wentToPenalties = result.wentToPenalties; fixture.result = result;
        cup.history.push({ ...fixture }); cup.pendingWinnerIds ||= [];
        if (!cup.pendingWinnerIds.includes(result.winnerClubId)) cup.pendingWinnerIds.push(result.winnerClubId);
        cup.eliminatedClubIds ||= []; const loser = result.winnerClubId === fixture.homeClubId ? fixture.awayClubId : fixture.homeClubId;
        if (!cup.eliminatedClubIds.includes(loser)) cup.eliminatedClubIds.push(loser);
    },
    _finishRoundIfReady(state, cup) {
        if (!cup || cup.matches.some(m => !m.played)) return false;
        const winners = [...new Set(cup.pendingWinnerIds || [])];
        cup.qualifiedClubIds = winners; cup.matches = []; cup.pendingWinnerIds = [];
        ...
        cup.roundIndex += 1; cup.currentRound = null; cup.roundMonth = ROUND_PLAN[Math.min(cup.roundIndex, ROUND_PLAN.length - 1)].month; if (winners.length === 2) cup.finalistIds = winners;
        return true;
    },
    getSummary(state, country = countryForPlayer(state)) {
        const cup = this.getCup(state, country), pid = playerClubId(state);
        if (!cup) return null;
        return { id: cup.id, name: cup.name, country: cup.country, round: cup.currentRound, roundMonth: cup.roundMonth, status: cup.status, champion: cup.champion, winnerId: cup.winnerId, playerStillIn: cup.status !== 'finished' && cup.qualifiedClubIds.includes(pid), history: cup.history.length, finalists: cup.finalistIds || [] };
    },
    finalizeSeason(state) {
        this.ensure(state); state.cupHistory ||= [];
        for (const cup of Object.values(state.cups)) state.cupHistory.push({ id: cup.id, name: cup.name, country: cup.country, seasonYear: cup.seasonYear, champion: cup.champion, winnerId: cup.winnerId, history: cup.history });
        state.cups = {}; return state.cupHistory;
    }
};
